...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        self.kpt_K = int(kpt_shape[0])  # Number of keypoints
        self.kpt_D = int(kpt_shape[1])  # Dimensions per keypoint

        # Get flip index from data_dict or use defaults
        if "kpt_flip_index" in data_dict:
            self.flip_index = data_dict["kpt_flip_index"]
        elif self.kpt_K == 17:
            # Use COCO defaults for K=17
            self.flip_index = [0, 2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15]
        else:
            # Identity mapping for non-COCO keypoint counts
            self.flip_index = list(range(self.kpt_K))
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            expected_cols = 5 + self.kpt_K * self.kpt_D
                            assert l.shape[1] == expected_cols, (
                                f"labels require {expected_cols} columns each (5 + {self.kpt_K}*{self.kpt_D})"
                            )
                            if self.kpt_D >= 3:  # Has visibility component
                                assert (l[:, 5::3] <= 1).all(), (
                            ...
                            if (
                                ...
                                for i in range(len(l)):
                                    ...
                                    kpts_without_vis[i] = np.hstack((l[i, :5], kpt))
                                l = kpts_without_vis
                                assert l.shape[1] == 5 + self.kpt_K * 2, (
                                    f"labels require {5 + self.kpt_K * 2} columns each after removing occlusion parameter"
                                )
                        else:
