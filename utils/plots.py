...
from utils.keypoints import KeypointSpec, coco_defaults
...
def plot_skeleton_kpts(
    im, kpts, steps, orig_shape=None, spec=None, kpt_shape=None, skeleton=None
):
    """Plot keypoints and skeleton with dynamic keypoint specification.

    Args:
        im: Image array
        kpts: Keypoint coordinates array
        steps: Number of dimensions per keypoint (typically 2 or 3)
        orig_shape: Original image shape
        spec: Optional KeypointSpec object containing K, D, skeleton, etc.
        kpt_shape: Optional tuple (K, D) as fallback
        skeleton: Optional skeleton connections as fallback
    """
    # Determine K, D and skeleton from spec or fallbacks
    if spec is not None:
        K, D = spec.K, spec.D
        skeleton_connections = spec.skeleton
    elif kpt_shape is not None:
        K, D = kpt_shape
        skeleton_connections = skeleton if skeleton is not None else []
    else:
        # Default to COCO values for backward compatibility
        coco_spec = coco_defaults()
        K, D = coco_spec.K, coco_spec.D
        skeleton_connections = coco_spec.skeleton

    # Generate a color palette sized to K keypoints
    ...
    palette = np.array([base_palette[i % len(base_palette)] for i in range(K)])

    # Generate colors for keypoints and limbs
    ...
    # Draw keypoints
    for kid in range(min(num_kpts, K)):  # Ensure we don't exceed K
        ...
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            # Check visibility if D >= 3 and steps includes visibility
            if steps >= 3 and D >= 3:
    ...
    for sk_id, sk in enumerate(skeleton_connections):
        ...
        # Ensure indices are valid (skeleton connections are 0-based in our KeypointSpec)
        kpt1_idx = sk[0]
        kpt2_idx = sk[1]
