...
def non_max_suppression(
    ...
    """
    # Compute dynamic keypoint dimensions
    if kpt_shape is not None:
        K, D = kpt_shape
        no_kpt = K * D
    else:
        # Infer keypoint dimensions from prediction shape
        if kpt_label:
            # Default fallback: assume D=3, then infer no_kpt
            if nc is None:
                # Try COCO default: nc=80, remaining for keypoints
                remaining = prediction.shape[2] - 5
                if remaining >= 131:  # 80 classes + 51 keypoint channels (17*3)
                    nc = 80
                    no_kpt = remaining - nc
                else:
                    nc = remaining
                ...
                no_kpt = max(prediction.shape[2] - (5 + nc), 0)
        else:
            no_kpt = 0

    if nc is None:
        nc = prediction.shape[2] - 5 - no_kpt
    ...
    t = time.time()
    output_size = 6 + no_kpt if kpt_label else 6
    output = [
        torch.zeros((0, output_size), device=prediction.device)
    ] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if not kpt_label:
                ...
                kpts = x[:, 5 + nc :]  # Extract keypoints dynamically
