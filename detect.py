...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print(pred[..., 4].max())
        # Apply NMS
        # Get model keypoint parameters
        K = getattr(model, "nkpt", 17)  # default to COCO
        D = getattr(model, "kpt_dim", 3)  # default to 3 dimensions
        nc = getattr(model, "nc", 80)  # default to COCO classes
        ...
        pred = non_max_suppression(
            ...
            kpt_label=kpt_label,
            nc=nc,
            kpt_shape=(K, D) if kpt_label else None,
        )
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        # Get keypoint spec from model if available
