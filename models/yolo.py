...
class Detect(nn.Module):
    ...
    def __init__(
        ...
        self.nkpt = nkpt if nkpt is not None else 17  # default to COCO
        self.kpt_dim = kpt_dim  # keypoint dimensions (default 3 for x,y,visibility)
        self.dw_conv_kpt = dw_conv_kpt
        self.no_det = nc + 5  # number of outputs per anchor for box and class
        self.no_kpt = (
            self.kpt_dim * self.nkpt
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        x_kpt[..., 0 :: self.kpt_dim] = (
                            x_kpt[..., 0 :: self.kpt_dim] * 2.0
                            - 0.5
                            + kpt_grid_x.repeat(1, 1, 1, 1, self.nkpt)
                        ) * self.stride[i]  # xy
                        x_kpt[..., 1 :: self.kpt_dim] = (
                            x_kpt[..., 1 :: self.kpt_dim] * 2.0
                            - 0.5
                        ...
                        if (
                            self.kpt_dim > 2
                        ):  # Apply sigmoid to visibility/confidence if present
                            x_kpt[..., 2 :: self.kpt_dim] = x_kpt[
                                ..., 2 :: self.kpt_dim
                            ].sigmoid()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        if m in [
            ...
            # Add kpt_dim parameter with precedence: model cfg -> default 3
            kpt_dim = d.get("kpt_dim", 3)
            args_dict["kpt_dim"] = kpt_dim
        elif m is ReOrg:
            c2 = ch[f] * 4
        elif m is Contract:
            c2 = ch[f] * args[0] ** 2
