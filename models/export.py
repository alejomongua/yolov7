...
if __name__ == "__main__":
    ...
    try:
        ...
        # Add keypoint metadata to ONNX model
        model_onnx = onnx.load(f)  # load onnx model

        # Get keypoint specification from model or use COCO defaults
        if hasattr(model, "keypoint_spec") and model.keypoint_spec is not None:
            spec = model.keypoint_spec
            print(f"{prefix} using model keypoint spec: K={spec.K}, D={spec.D}")
        else:
            # Default COCO specification
            from utils.keypoints import coco_defaults

            spec = coco_defaults()
            print(f"{prefix} using default COCO keypoint spec: K={spec.K}, D={spec.D}")

        # Add keypoint metadata to ONNX metadata_props
        metadata_props = {}
        metadata_props["kpt_shape"] = json.dumps([spec.K, spec.D])

        if spec.names:
            metadata_props["kpt_names"] = json.dumps(spec.names)

        if spec.skeleton:
            metadata_props["kpt_skeleton"] = json.dumps(spec.skeleton)

        if spec.flip_index:
            metadata_props["kpt_flip_index"] = json.dumps(spec.flip_index)

        if spec.oks_sigmas:
            metadata_props["oks_sigmas"] = json.dumps(spec.oks_sigmas)
        ...
        print(f"{prefix} added keypoint metadata: {list(metadata_props.keys())}")
