...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Build KeypointSpec from dataset and attach to model
    try:
        keypoint_spec = from_dataset_dict(data_dict)
        if not hasattr(model, "keypoint_spec"):
            model.keypoint_spec = keypoint_spec
    except Exception as e:
        logger.warning(
            f"Failed to create KeypointSpec from dataset: {e}. Using defaults."
        )

    # Attach keypoint dimensions from dataset if not already set
    if hasattr(model, "model") and len(model.model) > 0:
        last_layer = model.model[-1]
        if hasattr(last_layer, "nkpt") and hasattr(last_layer, "kpt_dim"):
            # Use spec if available, otherwise extract from dataset YAML
            if hasattr(model, "keypoint_spec"):
                K, D = model.keypoint_spec.K, model.keypoint_spec.D
            else:
            ...
            # Set model attributes for use in detect.py
            if not hasattr(model, "nkpt"):
                model.nkpt = K
            if not hasattr(model, "kpt_dim"):
                model.kpt_dim = D
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local-rank", type=int, default=-1, help="DDP parameter, do not modify"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--kpt-label", action="store_true", help="use keypoint labels for training"
    )
